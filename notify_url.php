...
if($verify_result) {//验证成功
	...
	if($api=='direct'){
    if($trade_status == 'TRADE_SUCCESS') {    //交易成功结束
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
			//如果有做过处理，不执行商户的业务程序
		$wpdb->update( $wpdb->alipay, array( 'alipay_price' => $total, 'alipay_email' => $buyer_email ,'alipay_time' => $gmt_payment, 'alipay_status' => '交易成功','alipay_dl' => 'true'), array( 'alipay_num' => $out_trade_no ) , array( '%f', '%s', '%s', '%s', '%s' ), array( '%s' )) ;
...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	//该判断表示买家已在支付宝交易管理中产生了交易记录，但没有付款
		$wpdb->update( $wpdb->alipay, array( 'alipay_price' => $total, 'alipay_email' => $buyer_email ,'alipay_time' => $gmt_payment, 'alipay_status' => '等待买家付款','alipay_dl' => 'false'), array( 'alipay_num' => $out_trade_no ) , array( '%f', '%s', '%s', '%s', '%s' ), array( '%s' )) ;
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
			//如果有做过处理，不执行商户的业务程序
	...
	//该判断表示买家已在支付宝交易管理中产生了交易记录且付款成功，但卖家没有发货
		$wpdb->update( $wpdb->alipay, array( 'alipay_price' => $total, 'alipay_email' => $buyer_email ,'alipay_time' => $gmt_payment, 'alipay_status' => '等待卖家发货','alipay_dl' => 'true'), array( 'alipay_num' => $out_trade_no ) , array( '%f', '%s', '%s', '%s', '%s' ), array( '%s' )) ;
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
			//如果有做过处理，不执行商户的业务程序
	...
	//该判断表示卖家已经发了货，但买家还没有做确认收货的操作
		$wpdb->update( $wpdb->alipay, array( 'alipay_price' => $total, 'alipay_email' => $buyer_email ,'alipay_time' => $gmt_payment, 'alipay_status' => '等待买家确认','alipay_dl' => 'true'), array( 'alipay_num' => $out_trade_no ) , array( '%f', '%s', '%s', '%s', '%s' ), array( '%s' )) ;
		//判断该笔订单是否在商户网站中已经做过处理（可参考“集成教程”中“3.4返回数据处理”）
			//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
			//如果有做过处理，不执行商户的业务程序
